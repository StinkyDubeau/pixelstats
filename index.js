...
function getKey(path) {
    // Returns text from 'path'. Creates 'path' if it doesn't exist.
    var key;
    var needKeyMsg =
        "- You have not inserted your API key into apikeys.txt. The app will not work until you do. -";
    try {
        ...
        try {
            fs.writeFile(path, needKeyMsg, (err) => {
                if (err) throw err;
                console.log(
                    `${path} has been created. Please paste your API key inside.`
                );
            });
        } catch (err) {
...
function translateJSON(original) {
    // Converts hypixel's JSON into something easier to work with.
    var translated = {
...
function validateUUID(uuid) {
    const Pattern =
        /^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[4|1][0-9a-fA-F]{3}-[8|9|aA|bB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$/;

    if (Pattern.test(uuid)) {
        return uuid; // Minecraft UUID is valid
    } else {
        console.error(
            "Malformed Minecraft UUID. Attempting to fix or using a default UUID."
        );
    ...
    if (HYPIXEL_KEY) {
        try {
            ...
            if (response.data.player == null) {
                ...
                console.log(
                    `${req.uuid} has never joined hypixel. Reverting to default UUID.`
                );
